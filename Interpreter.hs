...
run ((Fetch x): codigoRestante, stackRestante,state) = run (codigoRestante,getJustInt (findState state x):stackRestante,state)
run ((Store x): codigoRestante, value1:stackRestante,state) = run (codigoRestante,stackRestante,subsState state x value1)
